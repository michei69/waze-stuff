from waze_util import rescale_tile
import json
import random

with open("test/test.json", "r") as f:
    data = json.load(f)

def create_wzt(tile_id: str, lineData: bytes, pointData: bytes, pointId: bytes, lineRoute: bytes, lineSpeedAvg: bytes, lineExtType: bytes, lineExtId: bytes, lineExtLevelByLine: bytes, lineSpeedMax: bytes, lineAttribute: bytes, laneType: bytes):
    STR0 = b"\x00"
    STR1 = b"\x00\x00\x00\x00Street\x00"
    STR2 = b"\x00"
    STR3 = b"\x00\x00\x00\x00"
    STR4 = b"\x00\x00\x00\x00"
    STR5 = b"\x00\x00\x00\x00City\x00"
    STR6 = b""
    STR7 = b"\x00" + tile_id.encode("utf-8")# + b"\x00"

    SHAPE = b"\x00\x00\x00\x00"
    LINEDATA = lineData
    LINESUMMARY = b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    BROKEN = b""
    LINEROUNDABOUT = b"\x00\x00"
    
    POINTDATA = b"\x10\x27\x10\x27\x10\x27\x00\x00\x00\x00\x00\x00\x00\x00\x10\x27" + pointData
    POINTID = b"\xfe\xff\xff\xff\xfe\xff\xff\xff\xfe\xff\xff\xff\xfe\xff\xff\xff" + pointId
    LINEROUTE = lineRoute
    STREETNAME = b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    STREETCITY = b"\x00\x00\x00\x00"

    POLYGONHEAD = b"\x00\x00\x04\x00\x01\x00\x10\x00\x10\x27\x00\x00\x10\x27\x00\x00"
    POLYGONPOINT = b"\x00\x00\x01\x00\x02\x00\x03\x00"
    LINEREF = b""
    LINESPEEDAVG = lineSpeedAvg
    LINESPEEDID = b""
    LINESPEEDIDSMALL = b""

    RANGE = b"\x00\x00\x00\x00\x00\x00"
    ALERTDATA = b""
    SQUAREDATA = int(tile_id).to_bytes(4, "little") + b"\x00\x00\x00\x00\x3F\x32\x5E\x68"
    METADATAATTRIBUTE = b"\x01\x00\x09\x00\x0E\x00\x00\x00\x01\x00\x28\x00\x31\x00\x00\x00\x01\x00\x3C\x00\x44\x00\x00\x00"
    VENUEHEAD = b""
    VENUEID = b""

    LINEEXTTYPE = lineExtType
    LINEEXTID = lineExtId
    LINEEXTLEVELBYLINE = lineExtLevelByLine
    LINEEXTLEVEL = b"\x00"
    LINESPEEDMAX = lineSpeedMax

    POLYGONEX = b"\x00\x00\x00\x00"
    LINEATTRIBUTE = lineAttribute
    STREETID = b"\x20\x4A\x96\x02"

    BEACONPOS = b""
    BEACONID = b""
    BEACONEXPOS = b""
    BEACONEXID = b""
    BEACONEXMASK = b""

    LANETYPE = laneType
    LINENEWTYPE = b""
    EXTPROTOBUFDATA = b""

    def check(raw: bytes, size: int):
        assert len(raw) % size == 0

    check(LINEDATA, 8)
    check(LINESUMMARY, 62)
    check(BROKEN, 2)
    check(LINEROUNDABOUT, 2)
    check(POINTDATA, 4)
    check(POINTID, 4)
    check(SHAPE, 4)
    check(LINEROUTE, 4)
    check(STREETNAME, 10)
    check(STREETCITY, 4)
    check(STREETID, 4)
    check(POLYGONHEAD, 16)
    check(POLYGONPOINT, 2)
    check(LINESPEEDAVG, 2)
    check(LINEREF, 4)
    check(RANGE, 6)
    check(ALERTDATA, 16)
    check(SQUAREDATA, 12)
    check(METADATAATTRIBUTE, 8)
    check(VENUEHEAD, 16)
    check(VENUEID, 2)
    check(LINEEXTTYPE, 1)
    check(LINEEXTID, 4)
    check(LINEEXTLEVELBYLINE, 1)
    check(LINEEXTLEVEL, 1)
    check(LINESPEEDMAX, 2)
    check(LINEATTRIBUTE, 4)
    check(LANETYPE, 1)
    check(LINENEWTYPE, 1)
    check(POLYGONEX, 4)
    check(BEACONPOS, 4)
    check(BEACONID, 16)
    check(BEACONEXPOS, 4)
    check(BEACONEXID, 16)
    check(BEACONEXMASK, 1)
    check(EXTPROTOBUFDATA, 1)
    check(LINESPEEDID, 4)
    check(LINESPEEDIDSMALL, 2)
    check(LINEROUNDABOUT, 2)

    with open(f"temp/{tile_id}.wzt", "wb") as f:
        f.write(b"\x2e\x00\x00\x00")
        f.write(b"\x00\x00\x00\x00")
        data = b""
        ln = 0
        count = 0

        def add(thing: bytes):
            nonlocal data, ln, count
            data += thing
            ln += len(thing)
            f.write(ln.to_bytes(4, "little"))
            count += 1
            # print(f"{count}/46")

        add(STR0)
        add(STR1)
        add(STR2)
        add(STR3)
        add(STR4)
        add(STR5)
        add(STR6)
        add(STR7)
        
        add(SHAPE)
        add(LINEDATA)
        add(LINESUMMARY)
        add(BROKEN)
        add(LINEROUNDABOUT)
        add(POINTDATA)
        add(POINTID)
        add(LINEROUTE)
        add(STREETNAME)
        add(STREETCITY)

        add(POLYGONHEAD)
        add(POLYGONPOINT)
        add(LINEREF)
        add(LINESPEEDAVG)
        add(LINESPEEDID)
        add(LINESPEEDIDSMALL)

        add(RANGE)
        add(ALERTDATA)
        add(SQUAREDATA)
        add(METADATAATTRIBUTE)
        add(VENUEHEAD)
        add(VENUEID)

        add(LINEEXTTYPE)
        add(LINEEXTID)
        add(LINEEXTLEVELBYLINE)
        add(LINEEXTLEVEL)
        add(LINESPEEDMAX)

        add(POLYGONEX)
        add(LINEATTRIBUTE)
        add(STREETID)

        add(BEACONPOS)
        add(BEACONID)
        add(BEACONEXPOS)
        add(BEACONEXID)
        add(BEACONEXMASK)

        add(LANETYPE)
        add(LINENEWTYPE)
        add(EXTPROTOBUFDATA)

        f.write(data)

def generate(tile_id, scale = 0):
    actual_tile_id = str(tile_id)
    tile_id = str(tile_id) if scale == 0 else str(rescale_tile(tile_id, scale))
    if tile_id not in data:
        return False

    lineData = b""
    pointData = b""
    pointId = b""
    lineRoute = b""
    lineSpeedAvg = b""
    lineExtType = b""
    lineExtId = b""
    lineExtLevelByLine = b""
    lineSpeedMax = b""
    lineAttribute = b""
    laneType = b""

    def addPoint(x, y):
        nonlocal pointData, pointId
        pointData += x.to_bytes(2, "little")
        pointData += y.to_bytes(2, "little")
        pointId += b"\xfe\xff\xff\xff"
    
    def addLine(startPoint, endPoint, roadType: int):
        nonlocal lineData, lineRoute, lineSpeedAvg, lineExtType, lineExtId, lineExtLevelByLine, lineSpeedMax, lineAttribute, laneType
        lineData += startPoint.to_bytes(2, "little")
        lineData += endPoint.to_bytes(2, "little")
        lineData += b"\xff\xff\x00\x00"

        lineRoute += b"\x01\x01\x01\x01"
        lineSpeedAvg += b"\x00\x00"
        lineExtType += roadType.to_bytes(1, "little")
        lineExtId += random.randint(0, 0xfffffffe).to_bytes(4, "little")#b"\x00\x00\x00\x00"
        lineExtLevelByLine += b"\x00"
        lineSpeedMax += b"\x00\x00"
        lineAttribute += b"\x20\x00\x00\x00"
        laneType += b"\x00"

    for point in data[tile_id]["points"]:
        addPoint(point["x"], point["y"])
    for name in data[tile_id]["lines"]:
        line = data[tile_id]["lines"][name]
        for i in range(len(line["points"])):
            if i < len(line["points"]) - 1:
                addLine(line["points"][i] + 4, line["points"][i + 1] + 4, line["type"])

    create_wzt(actual_tile_id, lineData, pointData, pointId, lineRoute, lineSpeedAvg, lineExtType, lineExtId, lineExtLevelByLine, lineSpeedMax, lineAttribute, laneType)
    return True

if __name__ == "__main__":
    # generate(366799430)
    for tile_id in data:
        generate(tile_id)